...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, experiment_cfg: dict):
    ...
    class Model(torch.nn.Module):
        ...
        def forward(self, obs):
            with torch.inference_mode():
                output = self.policy.act({"observations": obs, "states": None}, role="policy")
                return output[-1]["mean_actions"]

    model = Model(runner.agent.policy)
    ...
    outputs = ort_sess.run(None, {"input": rand_inputs.cpu().numpy()})

    print("If following are the same, than success! - ")
