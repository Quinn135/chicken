...
class ChickenEnvCfg(DirectRLEnvCfg):
    ...
    original_observation_space = 8 + 8 * 2 + 4 + 3 + 3 + 3
    ...
    contact_cfgs: dict[str, ContactSensorCfg] = {
        ...
        "esc0": ContactSensorCfg(
            prim_path=f"/World/envs/env_.*{root_str}/escmountcenter",
            update_period=0.0,
            # filter_prim_paths_expr=["/World/ground.*"],
        ),
    ...
    contact_cfg_ft: ContactSensorCfg = ContactSensorCfg(
        prim_path=f"/World/envs/env_.*{root_str}/escmountcenter_6",
        update_period=0.0,
        filter_prim_paths_expr=[f"/World/envs/env_.*{root_str}/escmountcenter_7"],
    )
